...
exports.registerCustomer = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Đăng ký thành công! Kiểm tra Email để kích hoạt." });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Lỗi server" });
  }
...
// [MỚI] Xác thực token từ Cookie (để check xem user còn đăng nhập không)
exports.verifyAuth = async (req, res) => {
  try {
    ...
    if (!token) {
      return res
        .status(401)
        .json({ message: "Chưa đăng nhập", isAuthenticated: false });
    }

    // Verify token
    const decoded = jwt.verify(token, process.env.JWT_SECRET);

    // Lấy thông tin user từ DB để đảm bảo account vẫn còn active
    const [users] = await db.query(
    ...
    if (users.length === 0) {
      return res
        .status(401)
        .json({ message: "User không tồn tại", isAuthenticated: false });
    }
  ...
  try {
    ...
    transporter.sendMail(mailOptions, (err, info) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ message: "Không thể gửi email. Thử lại sau." });
      }
