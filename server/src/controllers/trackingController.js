...
exports.logVisit = async (req, res) => {
    try {
        ...
        // Lưu vào DB
        const sql = 'INSERT INTO access_logs (ip_address, user_agent) VALUES (?, ?)';
...
exports.getTrackingStats = async (req, res) => {
    try {
        // 1. Lấy tổng số lượt truy cập
        const [totalRows] = await db.query('SELECT COUNT(*) as total FROM access_logs');
        const totalVisits = totalRows[0].total;

        // 2. Lấy số lượt truy cập hôm nay
        const [todayRows] = await db.query('SELECT COUNT(*) as today FROM access_logs WHERE DATE(visit_time) = CURDATE()');
        const todayVisits = todayRows[0].today;

        // 3. Lấy danh sách 20 lượt truy cập gần nhất
        const [recentLogs] = await db.query('SELECT * FROM access_logs ORDER BY visit_time DESC LIMIT 20');
